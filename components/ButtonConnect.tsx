...
type ConnectModalProps = { close: () => void, isShown: boolean, };

const ConnectModal = ({ close, isShown }: ConnectModalProps) => {
  const { connect, connectors, error, isLoading, pendingConnector } = useConnect();

  return <div id="crypto-modal" tabIndex={-1} aria-hidden="true" style={{ background: "#0008" }} className={"fixed top-0 left-0 right-0 flex items-center justify-center z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full" + (isShown ? "" : " hidden")}>
...
const ButtonConnect = ({ text }: Props) => {
  ...
  return <>
    {isConnected ? <div>Connected to {ensName ?? address}</div> :
      <button
        type="button"
        className="focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900 my-6 w-36 h-12"
        onClick={() => setModalOpen((prev) => !prev)}>
        {text}
      </button>
    }
    <ConnectModal isShown={isModalOpen} close={() => setModalOpen(false)} />
  </>;
};
