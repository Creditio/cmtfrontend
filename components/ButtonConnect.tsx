...
const ConnectModal = ({ close, isShown }: ConnectModalProps) => {
  ...
  return (
    <div
      ...
      className={
        "fixed top-0 left-0 right-0 flex items-center justify-center z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full" +
        (isShown ? "" : " hidden")
      }
    >
...
const ButtonConnect = ({ text }: Props) => {
  ...
  return (
    <>
      {isConnected ? (
        <div>Connected to {ensName ?? shortAddress}</div>
      ) : (
        <button
          type="button"
